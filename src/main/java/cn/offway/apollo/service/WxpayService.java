...
public class WxpayService {
    ...
    public JsonResult trade_JSAPI_IOS(String outtradeno, String ip, String body, double amount, String openid) {
        try {
            if (wxpayProperties.getIsSandbox()) amount = 1.01D;
            // 支付金额 **金额不能有小数点,单位是分!!**
            BigDecimal price = new BigDecimal(amount);
            ...
            paraMap.put("openid", openid);
            // 设置请求参数(小程序ID)
            paraMap.put("appid", "wxa32e68e20b08bf5d");
            paraMap.put("trade_type", "JSAPI");
            // MD5运算生成签名，这里是第一次签名，用于调用统一下单接口
            String sign = PaymentKit.createSign(paraMap, wxpayProperties.getPaternerKey());
            paraMap.put("sign", sign);
            ...
            if ("SUCCESS".equals(return_code)) {
                // 返回的预付单信息
                String prepay_id = map.get("prepay_id");
                returnMap.put("appId", "wxa32e68e20b08bf5d");
                returnMap.put("nonceStr", nonceStr);
                returnMap.put("package", "prepay_id=" + prepay_id);
                returnMap.put("signType", "MD5");
                returnMap.put("timeStamp", timeStamp);
